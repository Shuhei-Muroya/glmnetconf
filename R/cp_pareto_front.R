...
cp_pareto_front <- function(data,T_hope=20,line=FALSE) {
  ...
  # Solve teh best hyperparameters
  ...
  return(list(Pareto_front=p, data = df, best_hyperparameters=best_hyperparameters))
}
...
#   )
#
#   df$Type <- factor(df$Type, levels = c("Not Pareto Front", "Pareto Front", "Best Hyperparameters"))
#
#   df_front <- df %>%
